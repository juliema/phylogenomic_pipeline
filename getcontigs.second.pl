...
`ls -l *exonerate2.csv.ed.sorted  >files`;
open FILES, "<files"; 
while (<FILES>) {
	if (/(\S+).exonerate2.csv.ed.sorted/) {
		my $inputfile=$1; 
		print "$inputfile\n";
		#$inputfile =~ s/.csv//g;
		...
		print "Inputfile\t$inputfile.exonerate2.csv.ed.sorted\n";
		...
		open FH, "<$inputfile.exonerate2.csv.ed.sorted";
		...
		for my $tax (@taxarray) {
			...
			open FH1, "<$inputfile.exonerate2.csv.ed.sorted";
			my $pos=0;
			while (<FH1>) {
